...
unsigned short btchip_apdu_hash_input_start() {
    ...
    if ((G_io_apdu_buffer[ISO_OFFSET_P2] == P2_NEW) ||
        ...
        if (G_io_apdu_buffer[ISO_OFFSET_P1] == P1_FIRST) {
            ...
            PRINTF("Coin kind: %d\n", G_coin_config->kind);
            if ((G_coin_config->kind == COIN_KIND_ZCASH) || (G_coin_config->kind == COIN_KIND_KOMODO) || (G_coin_config->kind == COIN_KIND_ZCLASSIC) || (G_coin_config->kind == COIN_KIND_BITCOINZ)) {
                PRINTF("COIN KIND is something like ZCash\n");
